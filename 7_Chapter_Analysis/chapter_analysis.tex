...
We perform the FC calculation by scanning over \twonustar signal rates. At each signal rate hypothesis, multiple Toy MC datasets are generated with the best fit background levels. For each toy MC dataset, the fit is performed and the $\chi^2$ test statistic value calculated. From these fits, the true test statistic distribution can be observed and the 90\% inclusion threshold calculated as a function of signal rate.

The $\chi^2$ scan performed on the real data with fixed signal rate is a single sample from the $\chi^2$ distributions created by the Feldman-Cousins technique. When the observed $\chi^2$ coincides with the 90\% inclusion threshold for a given signal rate, that signal rate is the 90\% C.L. upper limit. The result of the Feldman-Cousins calculation are shown in Figure \ref{fig:FC}. The 90\% C.L. upper limit on the $2\nu\beta\beta^\ast$ half-life is $T_{1/2}>4.25\times 10^{24} yrs$. This limit is slightly more conservative than the Wilks' Thoerem result as expected. 

\begin{figure}[htb!]
    \centering
    \includegraphics[scale=0.7]{FC_Final_Stat.pdf}
    \caption{The black curve shows the best fit test statistic with each fixed signal rate. The red curve shows the 90\% inclusion threshold for the generated ToyMCs at each signal rate. The intersection point shows which rate distribution is 90\% included by the data's test statistic value.}
    \label{fig:FC}
\end{figure}

\subsection{Discussion}
While preliminary, this analysis sets a world-leading limit on the $2\nu\beta\beta^\ast$ process. Surpassing EXO-200's limit of $T_{1/2}>1.4\times 10^{24}$ yrs\cite{exo200} and this experiment's previous limit set by KLZ-400 of $T_{1/2}>8.3\times 10^{23}$ yrs\cite{KLZ400}.

\begin{figure}[htb]
    \centering
    \includegraphics[scale=0.9]{limits_and_theory.png}
    \caption{A comparison of experimental limits of $2\nu\beta\beta^\ast$ to the $0^+_1$ excited state and some theoretical predictions taken from \cite{2nuex_predictions}. The length of the arrows are meaningless and are used to indicate a lower limit on the half-life.}
    \label{fig:2nuex_limits}
\end{figure}

Comparing it to a range of theoretical predictions from \cite{2nuex_predictions}, we find this latest result excluding the current QRPA calculations but falling short of the next cluster of theoretical calculations. Figure \ref{fig:2nuex_limits} shows how this and previous experimental limits compare to theoretical predictions. However, considering that previous measurements of $2\nu\beta\beta^\ast$ half-lives in lighter nuclei were much lower than predicted, these exclusion gains are significant.