...
The fitting model is a simplified version of the full spectral fit described in Chapter ~\ref{chapter:Analysis}. The simplified fit configuration is shown in Table~\ref{tab:k40_fit_config}. The fit model is configured to capture all the major features in the energy spectrum in the lower energy range. The fit is performed from $0.5--2.8$ MeV which contains the low energy $^{85}$Kr and $^{210}$Bi peaks, the $^{11}$C bump, the $^{136}$Xe double-beta decay spectrum and endpoint, and the $^{40}$K gamma peak. The model does not provide accurate estimates of each individual background since many are neglected, but the $^{40}$K peak is prominent and can be estimated. The fit is performed by the same maximum likelihood method as the full spectral fit in Chapter~\ref{chapter:Analysis}. The same $\chi^2$ is minimized with respect to the reduced parameters of this $^{40}$K analysis.

\begin{table}[htbp]
    \centering
    \caption{Fit parameter configuration for the $^{40}$K spectral analysis. The fit condition column indicates whether the parameter is free, fixed, scanned, or constrained in the fit.}
...
\begin{figure}[t!]
	\centering
	\includegraphics[scale=0.9]{k40_example_fit.png}
	\caption{An example $^{40}$K fit performed at $r=15m^3$}, outside the IB. 
	\label{fig:k40}
\end{figure}

The purpose of these fits is to measure the relative strength of the $^{40}$K electron-capture peak as a function of radius. Each fit of a radial shell returns a measured number of $^{40}$K events at that radius. The resulting radial dependence of the fitted $^{40}$K events is compared with the expected distribution obtained from KLG4Sim simulations. A least-squares ``fit of fits'' procedure is then used to extract the overall $^{40}$K activity associated with the inner balloon film. This fit of fits is shown in Figure \ref{fig:k40}


\begin{figure}[t!]
	\centering
	\includegraphics[scale=0.9]{k40fits.png}
	\caption{The fitted $^{40}$K radial distribution. Each point is the fitted $^{40}$K rate in a spherical section of the detector. The blue curve is the simulated radial distribution of $^{40}$K originated in the inner balloon film. Error increases on and inside the inner balloon film due to large $2\nu\beta\beta$ background.}
	\label{fig:k40}
\end{figure}
...
\hline
\end{tabular}
\caption{Production and background rate of carbon spallation products. Values from Reference~\cite{kamland_spallation_2009}.}
