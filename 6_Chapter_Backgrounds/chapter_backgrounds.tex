...
\begin{equation}
    Y_{K40\ film}= 183\pm 13 \text{events/day}
\end{equation}
...
\hline
 & $E$ [MeV] & $\tau$ & Prod. rate & Bkg. rate & Rej. eff. \\
...
From the exponential fit results of the dT distribution, the production rate of $^{11}C$ in XeLS can be estimated. The calculation of expected number of detected, selected, and correlated $^{11}$C-$\mu$ pairs is modeled as:
\begin{equation}
...
\end{equation}
\begin{itemize}
	\item $I_{C11}=10,028$: Integral of the exponential component of the fit, "Observed $\mu-^{11}$C pairs"  $[events]$
	\begin{equation}
	...
	\item $E_{FBE}=22.71$: XeLS Exposure $[kton\cdot days]$
	\item $dt_{MoG}=1.88\%$: MoGURA Deadtime Fraction
	\item $\epsilon_{dR}=57\%$: $dR<80$ cm cut efficiency (from FLUKA tuned with $^{11}$C), for each data period [unitless]
	\item $\epsilon_{dT}=94.5\%$: dT $>100$s cut efficiency (from known $^{11}$C half-life) [unitless]
	\item $\epsilon_{FV-E}=79.7\%$: Fiducial Volume \& Energy Cut Efficiency taken from KLG4Sim [unitless]
\end{itemize}

From the integrated observed events from the exponential component of the fit, and the estimation of the relevant efficiency factors, the production rate of $^{11}C$ in XeLS is calculated.
\begin{equation}
    Y_{C11}=1,050\pm 110\ events/kton\cdot day
\end{equation}

The uncertainty is estimated by considering a few major sources of uncertainty, the included uncertainties are listed in Table \ref{tab:c11_uncertainty}.
